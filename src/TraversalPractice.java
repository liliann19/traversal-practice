public class TraversalPractice {
  ...
  public static int maxVal(Node<Integer> node) {
    if (node == null)
      return 0;
    if (maxVal(node.left) > node.value) {
      node.value = maxVal(node.left);
    }
    if (maxVal(node.right) > node.value) {
      node.value = maxVal(node.right);
    }
  ...
  public static <T> int numLevels(Node<T> node) {
    ...
    if (numLevels(node.left) >= numLevels(node.right)) {
      levels += numLevels(node.left);
    } else {
      levels += numLevels(node.right);
    }
    return levels;
  }
